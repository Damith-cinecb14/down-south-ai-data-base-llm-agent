...
from resp_models import AgentQueryRequest, AgentQueryResponse, DMLProposalRequest, DMLProposalResponse, \
    DMLApprovalResponse, DMLApprovalRequest

router = APIRouter(prefix="/agent", tags=["Database Agent Routes"])


#Study about - MCP Server (Model Context Protocol)
...
def query_database(request:AgentQueryRequest) -> AgentQueryResponse:
    try:
        ...
        result = query_db_with_natural_language(
