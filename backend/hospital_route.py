...
from resp_models import HospitalCreate, HospitalResponse, HospitalUpdate
...
async def create_hospital(hospital_data:HospitalCreate,db:AsyncSession = Depends(get_db)): # dependency injection
    ...
    existing_hospital = await repo.get_hospital_by_name(hospital_data.name)

    if existing_hospital:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail="A hospital already exists with this name")
    if hospital_data.email:
        existing_email = await repo.get_hospital_by_email(hospital_data.email)
        if existing_email:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="A hospital already exists with this email")
...
async def update_hospital(hospital_id: int, hospital_data: HospitalUpdate, db: AsyncSession = Depends(get_db)):
    repo = HospitalRepository(db)
    hospital = await repo.get_hospital_by_id(hospital_id=hospital_id)
    if not hospital:
    ...
    if hospital_data.name and hospital_data.name != hospital.name:
        existing_name = await repo.get_hospital_by_name(hospital_data.name)
        if existing_name:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="A hospital already exists with this name")
    ...
    return await repo.update(hospital, hospital_data)
