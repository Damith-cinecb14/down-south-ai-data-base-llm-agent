...
from resp_models import HospitalCreate, HospitalUpdate
...
class HospitalRepository:
    ...
    async def get_hospital_by_name(self, name: str) -> Hospital | None:
        query = select(Hospital).where(Hospital.name == name)
        result = await self.db.execute(query)
        return result.scalars().first()

    async def update(self, hospital: Hospital, hospital_update: HospitalUpdate) -> Hospital:
